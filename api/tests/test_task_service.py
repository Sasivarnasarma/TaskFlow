...
def test_service_create_task_validation(db_session: Session):
    ...
    task = service.create_task(db_session, task_data)
    ...
    with pytest.raises(HTTPException) as exc_info:
        service.create_task(db_session, invalid_data)
...
def test_service_get_by_id_and_not_found(db_session: Session):
    ...
    created = service.create_task(db_session, task_data)
    created_id = cast(int, created.id)

    fetched = service.get_task_by_id(db_session, created_id)
    assert cast(int, fetched.id) == created_id

    with pytest.raises(HTTPException) as exc_info:
        service.get_task_by_id(db_session, 99999)
...
def test_service_delete_task(db_session: Session):
    ...
    service.delete_task(db_session, created_id)

    with pytest.raises(HTTPException) as exc_info:
        service.get_task_by_id(db_session, created_id)
    assert exc_info.value.status_code == 404

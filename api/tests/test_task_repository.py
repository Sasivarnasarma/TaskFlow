...
def test_create_and_get_by_id(db_session: Session):
    ...
    fetched = repository.get_by_id(db_session, created_id)
...
def test_get_all_filtering_and_sorting(db_session: Session):
    ...
    # Filter by status
    done_tasks = repository.get_all(db_session, status=TaskStatus.DONE.value)
    assert len(done_tasks) == 1
    assert str(done_tasks[0].title) == "Beta Task"

    # Filter by priority
    high_tasks = repository.get_all(db_session, priority=TaskPriority.HIGH.value)
    assert len(high_tasks) == 1
    assert str(high_tasks[0].title) == "Beta Task"

    # Filter by search
    searched_tasks = repository.get_all(db_session, search="Alpha")
    assert len(searched_tasks) == 1
    assert str(searched_tasks[0].title) == "Alpha Task"

    # Sort title asc vs desc
    asc_tasks = repository.get_all(db_session, sort="title_asc")
    assert str(asc_tasks[0].title) == "Alpha Task"

    desc_tasks = repository.get_all(db_session, sort="title_desc")
    assert str(desc_tasks[0].title) == "Beta Task"


def test_update_and_delete(db_session: Session):
    task = Task(title="To Update", priority=TaskPriority.LOW.value, status=TaskStatus.TODO.value)
    ...
    assert repository.get_by_id(db_session, created_id) is None
